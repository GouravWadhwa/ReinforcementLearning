...
class Agent () :
    ...
    def agent_start (self, state) :
        cos_theta, sin_theta, velocity = state

        tan_theta = sin_theta / (cos_theta + 1e-100)

        active_tiles = self.Tiles.coded_representation (tan_theta, velocity)
        current_action = self.agent_select_action (active_tiles)

        self.last_action = current_action
        self.previous_tiles = active_tiles

        return (self.last_action - 8)
